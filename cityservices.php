<?php
/*
Plugin Name: Chicago City Services Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago City Services Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagoCityServicesWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "city-services";
	
	public function ChicagoCityServicesWidget()
	{
		$widget_ops = array('classname' => 'ChicagoCityServicesWidget', 'description' => 'Embeds a Chicago City Services Widget' );
		$this->WP_Widget('ChicagoCityServicesWidget', 'Chicago City Services Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagoCityServicesWidget");') );

function display_chicago_CityServices_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_CityServices_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagoCityServicesWidget::$metro . "&schema=" . ChicagoCityServicesWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_CityServices(stripslashes($code));
}

function get_html_for_embed_chicago_CityServices($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_CityServices_widget', display_chicago_CityServices_widget);
